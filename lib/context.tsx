...
interface AppContextType {
  ...
  updateMenuItem: (item: Dish) => Promise<void>
  addMenuItem: (
    item: Omit<Dish, "id" | "comments" | "selections" | "created_at" | "updated_at" | "created_by">,
  ) => Promise<void>
  deleteMenuItem: (id: number) => Promise<void>
  getTranslatedDish: (dish: Dish) => Dish
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const updateMenuItem = async (item: Dish) => {
    try {
      ...
      if (response.ok) {
        ...
        setMenuItems(menuItems.map((dish) => (dish.id === item.id ? updatedDish : dish)))
      }
    } catch (error) {
  ...
  // 简化的翻译函数
  const getTranslatedDish = (dish: Dish): Dish => {
    try {
